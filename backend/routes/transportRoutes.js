...
router.get("/", async (req, res) => {
  try {
    ...
    const data = await Transport.find(filter).sort({ name: 1 });
    res.json(data);
...
router.post("/add", async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Transport created", data: created });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    res.json({ message: "Transport updated", data: updated });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.delete("/:id", async (req, res) => {
  try {
    const data = await Transport.findByIdAndUpdate(req.params.id, { isActive: false }, { new: true });
    if (!data) return res.status(404).json({ message: "Not found" });
    res.json({ message: "Transport deactivated", data });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
