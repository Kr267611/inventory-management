...
export default function LocationMaster() {
  ...
  const loadLocations = async () => {
    try {
      setLoading(true);
      const data = await LocationApi.getAllLocations();  // 👈 swap with locationApi.getAll() later
      setLocations(data);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      ...
      await LocationApi.updateLocation(editingId, form); // 👈 swap with LocationApi.update() later
    } else {
      const newLocation = { ...form, _id: `local-${Date.now()}`, createdAt: new Date().toISOString() };
      setLocations([newLocation, ...locations]);
      await LocationApi.createLocation(form); // 👈 swap with LocationApi.create() later
    }
    setDrawerOpen(false);
  }catch(err){
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Sure delete karna hai?")) {
      ...
      try {
        await LocationApi.deleteLocation(id); // 👈 swap with LocationApi.remove() later
      } catch (err) {
    ...
    <div className="location-page">
      ...
      <div className="location-page__header">
        <div className="location-page__title-wrap">
          ...
          <div className="location-breadcrumb">
            ...
            <span className="location-breadcrumb__current">Location</span>
          </div>
        </div>
        <button className="location-btn location-btn--primary" onClick={openAdd}>
          <Icon.Plus /><span>Add New Location</span>
        </button>
      </div>

      {/* Stats */}
      <div className="location-stat-card">
        <div className="location-stat-card__icon"><Icon.Location /></div>
        <div>
          <div className="location-stat-card__label">Total Locations</div>
          <div className="location-stat-card__value">{locations.length}</div>
          <div className="location-stat-card__hint">Total Location Records</div>
        </div>
      </div>
          ...
          <table className="location-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="4" className="location-td--empty">No locations found</td></tr>
              ) : (
                paginated.map((l, idx) => (
      ...
      <aside className={`location-drawer ${drawerOpen ? "location-drawer--open" : ""}`}>
        <div className="location-drawer__header">
          <h2 className="location-drawer__title">{editingId ? "Edit Location" : "Add New Location"}</h2>
        ...
        <div className="location-drawer__body">
          <section className="location-form-section">
            <h3 className="location-form-section__title">Location Information</h3>

            <Field label="Location Name" required>
              <input
                className="location-input"
                placeholder="e.g., Warehouse-1, Surat Godown, Mumbai Office"
                value={form.name}
