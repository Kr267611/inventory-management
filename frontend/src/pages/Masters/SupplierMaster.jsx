...
import { supplierApi } from "../../Api/SupplyApi";
...
export default function SupplierMaster() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        ...
        await supplierApi.update(editingId, form); // 👈 swap with supplierApi.update() later
      } else {
        const newSupplier = { ...form, _id: `local-${Date.now()}`, createdAt: new Date().toISOString() };
        setSuppliers([newSupplier, ...suppliers]);
        await supplierApi.create(form); // 👈 swap with supplierApi.create() later
      }
      setDrawerOpen(false);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Sure delete karna hai?")) {
      ...
      try {
        await supplierApi.remove(id); // 👈 swap with supplierApi.remove() later
      } catch (err) {
    ...
    <div className="location-page">
      ...
      <div className="location-page__header">
        <div className="location-page__title-wrap">
          <h1 className="location-page__title">Supplier </h1>
          <div className="location-breadcrumb">
            ...
            <span className="location-breadcrumb__current">Supplier</span>
          </div>
        </div>
        <button className="location-btn location-btn--primary" onClick={openAdd}>
          <Icon.Plus /><span>Add New Supplier</span>
        </button>
      </div>

      {/* Stats */}
      <div className="location-stat-card">
        <div className="location-stat-card__icon"><Icon.Supplier /></div>
        <div>
          <div className="location-stat-card__label">Total Suppliers</div>
          <div className="location-stat-card__value">{suppliers.length}</div>
          <div className="location-stat-card__hint">Total Supplier Records</div>
        </div>
      </div>
      ...
      <aside className={`location-drawer ${drawerOpen ? "location-drawer--open" : ""}`}>
        <div className="location-drawer__header">
          <h2 className="location-drawer__title">{editingId ? "Edit Supplier" : "Add New Supplier"}</h2>
        ...
        <div className="location-drawer__body">
          <section className="location-form-section">
            <h3 className="location-form-section__title">Supplier Information</h3>
            ...
            <Field label="Supplier Address" required>
              <input
                className="location-input"
